...
const state = {
  ...
  premium: { token: '', active: false, expiresAt: null },
};
...
const PREMIUM_STORAGE_KEY = 'paperly_premium_token';
const SPONSOR_MIN_USD = 2;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fetch GitHub stars
  // Fetch GitHub stars (shields.io has higher rate limits than raw GitHub API)
  fetch('https://img.shields.io/github/stars/SIDDHANTCOOKIE/paperly?style=social')
    .catch(() => {});
  fetch('https://api.github.com/repos/SIDDHANTCOOKIE/paperly')
    .then(r => r.ok ? r.json() : Promise.reject())
    .then(d => { document.getElementById('gh-stars').textContent = '\u2605 ' + (d.stargazers_count || 0); })
    .catch(() => { document.getElementById('gh-stars').textContent = '\u2605'; });
});
...
function getPremiumToken() {
  return localStorage.getItem(PREMIUM_STORAGE_KEY) || '';
}

function setPremiumToken(token) {
  if (!token) return;
  state.premium.token = token;
  localStorage.setItem(PREMIUM_STORAGE_KEY, token);
}
...
function restorePremium() {
  ...
  if (token) state.premium.token = token;
  updatePremiumUi();
}
...
function updatePremiumUi() {
  ...
  const sponsorBtn = $('#sponsorBtn');
  const expires = state.premium.expiresAt ? formatDate(state.premium.expiresAt) : '';

  if (state.premium.active) {
    if (badge) {
      badge.classList.remove('hidden');
      badge.textContent = expires ? `Premium active until ${expires}` : 'Premium active';
    }
    if (note) note.textContent = 'Premium unlocked: higher limits and priority access.';
    if (sponsorBtn) sponsorBtn.textContent = 'Manage Premium';
    return;
  }
  ...
  if (note) note.textContent = `Free tier: limited requests. Sponsor from $${SPONSOR_MIN_USD} to unlock Premium.`;
  if (sponsorBtn) sponsorBtn.textContent = 'Sponsor Project';
}

async function refreshPremiumStatus() {
  if (!state.premium.token) {
    updatePremiumUi();
    return;
  }
  try {
    const res = await fetch(`${API_BASE}/api/premium/status`, {
      ...
      mode: 'cors',
      credentials: 'omit',
      headers: { Authorization: `Bearer ${state.premium.token}` },
    });
    if (!res.ok) {
      clearPremiumToken();
      updatePremiumUi();
    ...
    state.premium.active = Boolean(data.active);
    state.premium.expiresAt = data.expiresAt || null;
  } catch {
    state.premium.active = Boolean(state.premium.token);
  }
  updatePremiumUi();
}
...
function sponsorMsg(text, isError = false) {
  const el = $('#sponsorMsg');
  if (!el) return;
  el.textContent = text || '';
...
async function createSponsorOrder(payload) {
...
async function verifySponsorPayment(payload) {
...
function initSponsorModal() {
  const openBtn = $('#sponsorBtn');
  const closeBtn = $('#sponsorCloseBtn');
  const modal = $('#sponsorModal');
  const form = $('#sponsorForm');
  const payBtn = $('#sponsorPayBtn');

  if (openBtn) {
    openBtn.addEventListener('click', () => {
      openBtn.textContent = 'Coming Soon';
      openBtn.disabled = true;
      setTimeout(() => { openBtn.textContent = 'Sponsor Project'; openBtn.disabled = false; }, 2000);
    });
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const sponsorName = $('#sponsorName')?.value.trim() || 'Sponsor';
    const sponsorEmail = $('#sponsorEmail')?.value.trim() || '';
    const amountUsd = Number($('#sponsorAmount')?.value || 0);

    if (!Number.isFinite(amountUsd) || amountUsd < SPONSOR_MIN_USD) {
      sponsorMsg(`Minimum sponsorship is $${SPONSOR_MIN_USD}.`, true);
      return;
    }

    payBtn.disabled = true;
    sponsorMsg('Creating secure checkout...');

    try {
      const order = await createSponsorOrder({ sponsorName, sponsorEmail, amountUsd });
      ...
      const checkout = new window.Razorpay({
        ...
        name: 'Paperly',
        description: `Sponsor Premium (${order.premiumDays} days)`,
        prefill: { name: sponsorName, email: sponsorEmail },
        notes: { sponsorAmountUsd: String(amountUsd) },
        theme: { color: '#f5f5f5' },
        handler: async (response) => {
          sponsorMsg('Verifying payment...');
          const verified = await verifySponsorPayment({
            ...response,
            sponsorEmail,
            amountUsd,
          });
          setPremiumToken(verified.premiumToken);
          state.premium.active = true;
          state.premium.expiresAt = verified.expiresAt || null;
          updatePremiumUi();
          sponsorMsg('Payment verified. Premium unlocked.');
          setTimeout(() => closeSponsorModal(), 900);
        },
        modal: {
          ondismiss: () => sponsorMsg('Payment window closed.', true),
        },
      });

      checkout.open();
    } catch (err) {
      sponsorMsg(err.message || 'Payment failed.', true);
    } finally {
...
async function api(endpoint, paperText, maxRetries = 2) {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      const token = state.premium.token || getPremiumToken();
      ...
      if (res.status === 429) {
        ...
        if (data.error && data.error.toLowerCase().includes('rate limit exceeded')) {
          alert(data.error);
